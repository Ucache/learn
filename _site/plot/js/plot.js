...
plotapp.controller('plotController', function($scope) {
    ...
    $scope.processUploadData = function() {
        ...
        r.onloadend = function(e) {
            $scope.chartData = e.target.result;
    ...
    $scope.verifyData = function() {
        ...
        if (lines.length <= 1) {
            this.errorInfo = "no input data, or need more data"
            return
        }
        var len = lines[0].split(",").length
        if (len > 3) {
            this.errorInfo = "demition larger than 2 is not support"
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (line.length != len) {
                this.errorInfo = "bad data format"
                this.highchartsNG.errors = "has-error"
                this.danger = "danger"
                return
            }
    ...
    $scope.calRegressLine = function() {
        ...
        $scope.highchartsNG.series.push({
            type: "line",
            data: [
                [xzero, parseFloat(theta[0]) + parseFloat(xzero * theta[1])],
                [xone, parseFloat(theta[0]) + parseFloat(xone * theta[1])]
            ]
        })
    }
